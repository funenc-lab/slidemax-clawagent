...
This directory is a production-ready OpenClaw agent workspace for presentation strategy, deck writing, slide review, speaker notes, and delivery execution.

It follows the OpenClaw workspace model: the workspace is the agent's home, prompt layer, local memory surface, and task operating context. The Git repository root is the local `slidemax-clawagent` repository root, while the actual workspace root is `agents/ppt/`. This directory is the workspace. It is not the SlideMax companion application and it is not the OpenClaw runtime itself.
...
- a dynamic installation path to the canonical SlideMax workflow used for actual PPT generation
...
For any task that requires an actual PPT, PPTX, SVG, or generated deck artifact, SlideMax and the canonical `slidemax-workflow` skill are mandatory prerequisites.
Without them, the workspace may still produce outlines, reviews, speaker notes, and delivery plans, but actual PPT generation is blocked.
...
- the local file `skills/message-channel-delivery/SKILL.md` handles post-delivery handoff to chats, groups, or message channels
...
If the workspace repository is missing locally, clone it first, then enter `agents/ppt` before following the runbook.
...
The workspace-root `outputs/` directory is only a staging area for generated files.
...
Generated local artifacts should be written under the workspace-root `outputs/` directory unless the user explicitly requests a different path.
...
- avoid writing generated deliverables to the workspace root
...
This workspace directory should contain workspace instructions, skills, validation scripts, and reusable generated outputs. It should not be treated as a credential store.
...
- this workspace path is the intended OpenClaw workspace location
